...
`<Link>`는 HTML `<a>` 요소를 확장한 React 컴포넌트로, [프리페치](/docs/app/getting-started/linking-and-navigating#prefetching)와 라우트 간 클라이언트 사이드 내비게이션을 제공합니다. Next.js에서 라우트 이동의 기본 방식입니다.
...
다음은 `<Link>` 컴포넌트에 전달할 수 있는 props입니다:
...
> **참고**:
>
> - Next.js는 탐색 시 페이지가 뷰포트에 보이지 않으면 [Page](/docs/app/api-reference/file-conventions/page)로 스크롤합니다.
...
- **`"auto"` 또는 `null` (기본값)**: 사전 로딩 동작은 라우트가 정적 또는 동적인지 여부에 따라 다릅니다. 정적 라우트의 경우 전체 라우트가 사전 로딩됩니다(모든 데이터 포함). 동적 라우트의 경우 [`loading.js`](/docs/app/api-reference/file-conventions/loading#instant-loading-states) 경계가 있는 가장 가까운 세그먼트까지 부분적으로 사전 로딩됩니다.
...
- `false`: 뷰포트 진입 시에는 사전 로딩하지 않지만 호버 시에는 사전 로딩됩니다. 호버 시 사전 로딩까지 완전히 제거하려면 `<a>` 태그를 사용하거나 [점진적으로 App Router를 도입](/docs/app/guides/migrating/app-router-migration)하세요.
