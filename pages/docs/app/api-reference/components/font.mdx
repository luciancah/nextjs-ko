...
[`next/font`](/docs/app/api-reference/components/font)는 커스텀 폰트를 포함한 폰트를 자동으로 최적화하고, 외부 네트워크 요청을 제거해 개인정보 보호와 성능을 개선합니다.

모든 폰트 파일에 대해 **내장 자동 self-hosting**을 제공하며, [레이아웃 시프트](https://web.dev/articles/cls) 없이 웹 폰트를 로드할 수 있습니다.

또한 [Google Fonts](https://fonts.google.com/)를 편리하게 사용할 수 있습니다. CSS와 폰트 파일은 빌드 시점에 다운로드되어 정적 에셋과 함께 self-hosting 됩니다. **브라우저에서 Google로 요청이 전송되지 않습니다.**
...
// variable 폰트를 로드하면 weight를 지정할 필요가 없습니다.
const inter = Inter({
  subsets: ['latin'],
  display: 'swap',
})
...
모든 페이지에서 폰트를 사용하려면 `/pages`의 [`_app.js` 파일](/docs/pages/building-your-application/routing/custom-app)에 다음과 같이 추가하세요:
...
> **🎥 시청:** `next/font` 사용법 더 알아보기 → [YouTube (6분)](https://www.youtube.com/watch?v=L8_98i_bMMA)
