...
메타데이터를 정의하려면 `metadata` 객체 또는 `generateMetadata` 함수를 사용할 수 있습니다.
...
`generateMetadata`를 해결하는 과정은 페이지 렌더링의 일부입니다. 페이지가 사전 렌더링 가능하고 `generateMetadata`가 동적 동작을 유발하지 않으면, 결과 메타데이터는 페이지의 초기 HTML에 포함됩니다.

그 외 경우에는 `generateMetadata`에서 해결된 메타데이터가 초기 UI 전송 이후 [스트리밍될 수 있습니다](/docs/app/api-reference/functions/generate-metadata#streaming-metadata).
...
`params`와 `searchParams`의 타입 자동완성을 위해, 첫 번째 인자에 페이지는 [`PageProps<'/route'>`](/docs/app/api-reference/file-conventions/page#page-props-helper), 레이아웃은 [`LayoutProps<'/route'>`](/docs/app/api-reference/file-conventions/layout#layout-props-helper)를 사용할 수 있습니다.

> **알아두면 좋은 정보**:
>
> - 메타데이터는 `layout.js`와 `page.js` 파일에 추가할 수 있습니다.
> - Next.js는 메타데이터를 자동으로 해석하고, 페이지에 필요한 `<head>` 태그를 생성합니다.
> - `metadata` 객체와 `generateMetadata` 함수는 **서버 컴포넌트에서만 지원**됩니다.
> - 동일한 경로 세그먼트에서 `metadata` 객체와 `generateMetadata` 함수를 동시에 내보낼 수 없습니다.
> - `generateMetadata` 내부의 `fetch` 요청은 `generateMetadata`, `generateStaticParams`, Layouts, Pages, Server Components 간에 동일 데이터에 대해 자동 [메모이제이션](/docs/app/guides/caching#request-memoization)됩니다.
> - `fetch`를 사용할 수 없는 경우 React [`cache`를 사용할 수 있습니다](/docs/app/guides/caching#react-cache-function).
> - [파일 기반 메타데이터](/docs/app/api-reference/file-conventions/metadata)가 더 높은 우선순위를 가지며 `metadata` 객체와 `generateMetadata` 함수보다 우선 적용됩니다.
...
- `props` - 현재 경로의 매개변수를 포함하는 객체:
  ...
  - `params` - 루트 세그먼트부터 `generateMetadata`가 호출된 세그먼트까지의 [동적 경로 매개변수](/docs/app/api-reference/file-conventions/dynamic-routes) 객체를 포함하는 객체입니다. 예시:
...
> - `fetch` 요청은 `generateMetadata`, `generateStaticParams`, 레이아웃, 페이지 및 서버 컴포넌트 간에 동일한 데이터를 위해 자동으로 [메모이제이션](/docs/app/guides/caching#request-memoization)됩니다. `fetch`가 사용 불가능한 경우, React [`cache`를 사용할 수 있습니다](/docs/app/guides/caching#react-cache-function).
