...
`updateTag`를 사용하면 [Server Actions](/docs/app/guides/caching) 내부에서 특정 캐시 태그에 대한 [cached data](/docs/app/getting-started/updating-data)를 온디맨드로 업데이트할 수 있습니다.

이 함수는 사용자가 변경 사항(게시물 생성 등)을 만들었을 때, 오래된 데이터 대신 UI가 즉시 변경 사항을 보여주는 **read-your-own-writes** 시나리오를 위해 설계되었습니다.

## Usage

`updateTag`는 **오직** [Server Actions](/docs/app/getting-started/updating-data) 내부에서만 호출할 수 있습니다. Route Handlers, Client Components, 또는 다른 어떤 컨텍스트에서도 사용할 수 없습니다.

Route Handlers나 다른 컨텍스트에서 캐시 태그를 무효화해야 한다면, 대신 [`revalidateTag`](/docs/app/api-reference/functions/revalidateTag)를 사용해야 합니다.
...
- 외부 API 요청 캐싱을 위해 `fetch`와 함께 [`next.tags`](/docs/app/guides/caching#fetch-optionsnexttags-and-revalidatetag) 옵션 사용:

```tsx
fetch(url, { next: { tags: ['posts'] } })
```

- `'use cache'` 지시어가 있는 캐시 함수나 컴포넌트 내부에서 [`cacheTag`](/docs/app/api-reference/functions/cacheTag) 사용:
...
- [`revalidateTag`](/docs/app/api-reference/functions/revalidateTag) - Route Handlers에서 태그를 무효화하기 위해
- [`revalidatePath`](/docs/app/api-reference/functions/revalidatePath) - 특정 경로를 무효화하기 위해
