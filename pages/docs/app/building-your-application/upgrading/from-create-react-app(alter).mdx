...
If you're using Tailwind, install `@tailwindcss/postcss`:

```bash filename="Terminal"
npm install -D tailwindcss @tailwindcss/postcss postcss
```

Create a `postcss.config.mjs` file at the root of your project:
...
Create an `app/globals.css` and import Tailwind:
